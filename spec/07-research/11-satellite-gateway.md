# Research: Satellite Gateway

## Problem

The MVP Satellite is a single node - one LiveKit instance and one token service. This is sufficient for small communities, but a single node has a hard ceiling: one server's bandwidth and CPU. When a community outgrows that ceiling, the operator needs to add nodes. The moment there are multiple nodes, a new problem emerges: **routing**.

A client hitting `/session/create` needs to land on a node with available capacity. A client hitting `/session/join` needs to land on the node that actually hosts the target room. Today, the `+orbit/sat-invite` tag embeds the Satellite URL, and the Satellite *is* the single node - so routing is trivial. With multiple nodes, something needs to sit in front and make routing decisions. That something is the **gateway**.

## Proposal

A thin routing layer - the Satellite gateway - that sits in front of a pool of LiveKit nodes and exposes the same API surface as a single-node Satellite. Clients see no difference. The gateway absorbs the token service role and adds room-to-node routing.
...
The gateway must track which rooms are on which nodes. This is a small, fast-changing dataset (room ID -> node URL). Options:

- **In-memory hash map** - simplest. Lost on gateway restart, but rooms are ephemeral anyway - a gateway restart means all routing state is rebuilt by querying each node's LiveKit API for active rooms.
- **Redis / shared store** - needed only if the gateway itself needs to be horizontally scaled (multiple gateway instances). Likely overkill for most deployments.
- **LiveKit's native room list API** - the gateway can query each node's LiveKit API on startup and periodically to rebuild its routing table. This makes the gateway stateless - it derives routing from the nodes themselves.
...
Start with **participant count** - it's available directly from LiveKit's API with no extra instrumentation. Refine to bandwidth or CPU-aware routing if participant count proves too coarse.
...
- **Complexity vs. value.** Most Orbit communities will fit on a single node. The gateway is engineering effort that serves only large deployments. If the community never grows past single-node scale, this work has no impact.
- **LiveKit Cloud.** LiveKit offers a hosted, managed multi-node solution. If operators are willing to use LiveKit Cloud instead of self-hosting, the gateway is unnecessary - LiveKit Cloud handles all routing internally. The gateway is only needed for fully self-hosted multi-node deployments.

## Evaluation Criteria

1. Prototype a gateway that routes between 2–3 local LiveKit instances.
2. Verify client transparency - an Orbit client connected to the gateway should behave identically to one connected to a single node. No client changes required.
3. Test scale-up: add a node while sessions are active, verify new sessions route to it.
4. Test drain: stop routing to a node, verify existing sessions continue, verify the node is retired once empty.
5. Test failure: kill a node, verify the gateway removes it from routing and clients on that node see a clean error.
6. Measure overhead: gateway should add <5ms to `/session/create` and `/session/join` latency.

## Dependencies

- Stable single-node Satellite (MVP)
...
Post-MVP. The single-node Satellite is sufficient for MVP communities. The gateway becomes relevant when operators report that a single node is insufficient - driven by real demand, not speculative scaling.
