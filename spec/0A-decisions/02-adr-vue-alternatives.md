...
The Orbit client needs a frontend framework for both the Tauri desktop shell and the web app / PWA. The framework drives UI rendering, state management, and component architecture across all three runtime contexts: desktop, web, and widget.

See [../04-clients/01-desktop.md](../04-clients/01-desktop.md) for the desktop client specification.
...
Given that trajectory, the practical performance delta is already small and shrinking. The team's existing Vue expertise and the shared Hivecom toolchain outweigh the residual advantage Svelte holds today.
...
For the MVP, raw Vite + `vite-plugin-pwa` keeps the toolchain minimal and VUI intact. Quasar may be revisited if monorepo build complexity grows to a point where its orchestration value outweighs the component library conflict.

## Decision Rationale

1. **Team expertise.** The Hivecom team has extensive experience with Vue 3 + Vite, making it the lowest-friction choice for the desktop frontend.
...
3. **Shared Hivecom toolchain.** Vue + Vite + VUI is used across other Hivecom projects, meaning shared tooling, shared components, and faster onboarding for new contributors.
4. **Composition API.** Vue 3's Composition API with `<script setup>` provides a productive, ergonomic development pattern that aligns with the team's existing code style across all Hivecom projects.
