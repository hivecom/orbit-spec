...
There are two build outputs: the Tauri binary (desktop) and the web app / PWA (web). There are three runtime contexts: desktop, web, and embedded. All three are served from the same `packages/core` source with a different platform adapter from `packages/platform`.
...
If an app (or another package) wants to consume exports from a specific package, it simply needs to reference it in its own `package.json`.
