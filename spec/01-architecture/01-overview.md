...
The system is composed of four independent named layers. **Uplink** is the IRC backbone - an Ergochat instance running IRCv3, handling text messaging, presence, channel state, and signaling. The MVP runs on stock Ergochat; the **Uplink fork** - a purpose-built IRCd that extends Ergo while remaining 100% IRC-compatible - is the primary development track and where the platform's full feature set lands. **Satellite** is the real-time media layer - an independent WebRTC service (LiveKit SFU) handling voice, video, and streaming. **Depot** is the storage layer - S3-compatible object storage for file uploads and avatars. A fourth named role, **Transponder**, refers to any OIDC-compliant identity provider the operator deploys; both Uplink and Satellite can consume it independently for identity verification. Deployments without a Transponder degrade gracefully to Ergochat's built-in NickServ/SASL.
