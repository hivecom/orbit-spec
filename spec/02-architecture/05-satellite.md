...
deployed as a container pair. Like a TeamSpeak or Mumble server, it's one
process hosting many rooms. Scaled deployments put multiple nodes behind a
gateway (see [Scaling](#scaling)).
...
It exists for in-session coordination, quick callouts during a call and
links during a screen share. Persistent, searchable chat lives in Uplink.
...
The metadata endpoint requires no authentication: unauthenticated clients
(the embedded client, anonymous users) need to discover and display
available voice sessions before authenticating. This is
...
unknown Satellite for the first time, the same way SSH clients confirm an
unknown host key. Once accepted, the client remembers the decision. The
...
exist) but not freely joinable, which suits private meetings. Protection is
...
handshake: one offer tag and one answer tag, two IRC messages total, each
carrying compact connection credentials (ICE credentials, DTLS fingerprint,
one candidate). All further negotiation happens over the WebRTC data channel,
...
retained trace is the handshake signaling in IRC, which is connection
metadata. Users who don't want the operator to see connection
...
- **With an identity provider** ([Transponder](07-transponder.md)): the token
  ...
  published keys, standard OIDC consumption with no custom protocol. Verified
...
gone. If the creator wants the same setup next time, their client provides
it again (the client may store those preferences locally as a convenience).
Creator and moderator enforcement uses LiveKit's native room-admin grant:
mute and kick are built-in LiveKit operations, not custom Orbit logic.
Delegated moderation requires verifiable identity: unverified users can't
receive it.
...
waits, and keeps no state beyond the knock's TTL. It's a doorbell, not a
waiting room. The knock delivery and status mechanics live in
[Implementation - Satellite](../03-implementation/03-satellite.md).

## Standalone Usage

Satellites work without Uplink entirely. The bootstrapping mechanism is a
direct `satellite://` link, shared out-of-band, that the client resolves to
the Satellite's token service. The
...
Room migration is out of scope: WebRTC sessions are tied to a
...
LiveKit Cloud offers hosted multi-node routing.
