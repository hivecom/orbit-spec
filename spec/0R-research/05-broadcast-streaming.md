...
This track explores integrating one-to-many broadcast livestreaming into the Orbit ecosystem as an optional add-on service, distinct from [Satellite](../02-architecture/05-satellite.md). The reference implementation under consideration is [qwer](https://github.com/hivecom/qwer), a minimalistic Rust livestreaming server that powers [qwer.ee](https://qwer.ee), built by the same team.

## Problem

Satellite is a bidirectional, low-latency media layer built on LiveKit SFU. It's designed for interactive use cases: voice calls, group video rooms, and screen sharing between participants. It isn't designed for broadcast, the "one streamer, many passive viewers" model common to platforms like Twitch or YouTube Live.
...
Broadcast is also a different shape architecturally: it's unidirectional, tolerates higher latency (2-10s is acceptable for most live content), and benefits from CDN delivery for large audiences. These properties call for a different component entirely.
...
qwer does **not** have a horizontal scaling path. It's a single-process, single-node server. All viewers connect to the same process; the process ships the full bitstream to each viewer individually over its own outbound connection. There is no:
...
For modest concurrent viewership (tens to low hundreds of viewers per stream) on a well-provisioned machine, this works. A 3 Mbps stream with 100 viewers consumes ~300 Mbps of egress from one host, which an ordinary VPS can sustain. The bottleneck is network bandwidth, not CPU or memory, and Tokio handles the async I/O efficiently.

For larger audiences, the standard scaling path would need to be built on top of qwer:

1. HLS/DASH segmentation: write fMP4 segments to an S3-compatible store, serve via CDN; ingest stays on one node, viewers pull from edge
...
Broadcast streaming is **not part of Orbit's core architecture**. Satellite handles interactive media; there is no gap in the core design that broadcast streaming fills. The two serve different use cases:
...
Before promoting this to a named component or investing in scaling work, assess:
...
- [Satellite](../02-architecture/05-satellite.md) - defines what Orbit's interactive media layer already covers and what it doesn't
- [Depot](../02-architecture/06-depot.md) - natural segment store if HLS segmentation is pursued
- [Transponder](../02-architecture/07-transponder.md) - identity layer for stream key validation
- [Uplink](../02-architecture/03-uplink.md) - IRC signaling path for stream-live notifications
